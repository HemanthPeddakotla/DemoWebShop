...
public class TC_009 extends BaseUnit{
	...
	public void removingProductFromCart() {
		...
		//4.Clicking on shoppingcart button
		DigitalDownloadsPage shoppingcart = new DigitalDownloadsPage(driver);
		shoppingcart.getShoppingcartButtton().click();
		
		String actualShoppingcartPageTitle = driver.getTitle();
		String expectedShoppingcartPageTitle = ReadData.fromProperty("ExpectedShoppingcartPageTitle");
		
		Assert.assertEquals(actualShoppingcartPageTitle, expectedShoppingcartPageTitle, "Shoppingcart page is not displayed");
		Reporter.log("Shoppingcart page is displayed",true);
		//5.Removing product from cart
		explicitWait.until(ExpectedConditions.visibilityOfElementLocated(By.name("removefromcart")));
		shoppingcart.getRemoveCheckbox().click();
		shoppingcart.getUpdateShoppingcartButton().click();
		String actualEmptyShoppingcartMessage = driver.findElement(By.xpath("//div[contains(text(),'Your Shopping Cart is empty!')]")).getText();
		Reporter.log(actualEmptyShoppingcartMessage,true);
		actualEmptyShoppingcartMessage=actualEmptyShoppingcartMessage.trim();
		
		String expectedEmptyShoppingcartMessage = ReadData.fromProperty("ExpectedEmptyShoppingcartMessage");
		
		Assert.assertEquals(actualEmptyShoppingcartMessage, expectedEmptyShoppingcartMessage, "Product is not removed from cart");
