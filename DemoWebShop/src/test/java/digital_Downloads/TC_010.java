...
public class TC_010 extends BaseUnit{
	...
	public void checkoutPage() {
		//1.Click on Digital download category
				...
				//5.Clicking on checkout button
				explicitWait.until(ExpectedConditions.visibilityOfElementLocated(By.name("termsofservice")));
				product.getTermsAndConditionsCheckbox().click();
				product.getCheckoutButton().click();
				
				String actualCheckoutPageTitle = driver.getTitle();
				String expectedCheckoutPageTitle = ReadData.fromProperty("ExpectedCheckoutPagetitle");
				
				Assert.assertEquals(actualCheckoutPageTitle, expectedCheckoutPageTitle, "Checkout page is not displayed");
